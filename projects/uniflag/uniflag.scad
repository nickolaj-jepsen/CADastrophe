...
//  6x M4x14 + nuts. M5 socket heads sink into flange counterbores from
...
//  supports and no bridges anywhere: the wall leans `yaw` deg
//  (self-supporting), the M5 wall bores are teardropped, and the frame
//  prints back-face down so its notches open upward.
// =====================================================================
...
usb_y     = [17, 29];   usb_nd = 9;   // USB tunnel depth (boot reaches z 7.5)
...
m5_rows   = [8, 186];  // board-y of the bolt rows (straddle the USB tunnel)
m5_hole   = 5.5;
m5_len    = 30;
...
echo(str("frame envelope: ", fl_edge + m4_c + m4_boss / 2, " x ", 2 * out_half));
...
assert(m5_rows[0] + cb_d / 2 <= usb_y[0] - 1, "row 1 crowds the USB tunnel");
...
assert(board_y0 + m5_rows[0] > 10 && board_y0 + m5_rows[1] < pl_h - 10,
       "M5 rows overrun the wall height");
...
// ---------------------------------------------------------------------
// frame — prints BACK-face down (the bed face is the wall contact plane,
// so every front-opening notch leaves a printable floor, never a bridge)
// ---------------------------------------------------------------------
module frame() {
    diff() {
        linear_extrude(fr_d)
        ...
        for (p = m5_pts) tag("remove") translate(p) {
            translate([0, 0, -lap]) cylinder(d = m5_hole, h = fr_d + 2 * lap);
            translate([0, 0, -lap]) cylinder(d = cb_d, h = cb_dep + lap);
            translate([0, 0, fr_d - foot_ch])
                cylinder(d1 = m5_hole, d2 = m5_hole + 2 * (foot_ch + eps),
                         h = foot_ch + eps);
        }
...
module wall_unrot() {
    ...
    tag_scope() diff() {
        ...
        for (y = [board_y0 + 10, board_y0 + 30]) tag("remove")
            translate([wall_x0 - lap, y, pl_t + 3])
    ...
    for (y = [pl_h / 2, pl_h / 2 - 55, pl_h / 2 + 55])
        translate([0, y + 3, 0])
            rotate([90, 0, 0])
                linear_extrude(6)
                    polygon([[wall_x0 + 1, -4], [wall_x0 + 1, pl_t + 16],
                             [wall_x0 - 18, -4]]);
}
...
// frame +z points into the wall, and the frame's back plane (z = fr_d)
// lands on the wall's driver-side face.
...
if (part == "bracket") bracket();
    ...
    up(fr_d) yrot(180) frame();   // back-face down
    translate([245, 0, 0]) ring();
}
