...
clamp ring; right edge open for the light sensor). Render/verify one body
...
boot), `yaw=15`, `chan_slack=0.35` (board float), `bd_clr=0.4`,
...
6× M4×14 + nuts (ring → frame; nuts drop into the back-opening pockets).
...
left-corner pockets once mounted) → frame onto the wall, M5×30s through the
...
rail-face down (15° wall is self-supporting, M5 bores teardropped), frame
...
40–50 % infill, **no supports, no bridges**. Use a 5 mm brim on the frame
